...
const callPrimaryApi = async (message: string, threadId?: string) => {
  try {
    const url = threadId ? `http://localhost:3001/chat/${threadId}` : primaryApiUrl;
    ...
    // If no threadId is available, retrieve it from the first response
    if (!threadId && data.threadId) {
      return { data, threadId: data.threadId };
    }

    return { data, threadId }; // Return the existing threadId
  } catch (error) {
...
const Chat = () => {
  ...
  const sendMessage = async () => {
    if (messageInput.trim()) {
      ...
      // Update threadId if it's the first response
      if (newThreadId && !threadId) {
        setThreadId(newThreadId);
      }

      // Handle the API response or fallback API call
